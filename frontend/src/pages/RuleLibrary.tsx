...
import { ruleLibraryApi, rulesApi, proposalsApi } from '../api/client'
import RuleChatPanel from './RuleChatPanel'
import { fmtIST } from '../utils/dates'
import type { RuleDefinition, RuleInstance, Rule, PendingProposal } from '../api/client'
import {
    ...
    <div className="px-6 py-4">
      <div className="flex items-start gap-4">
        ...
        <div className="flex flex-col items-end gap-2 flex-shrink-0">
          ...
          {/* Anomaly instances carry target_config.metric_name + asset_id; deep-link
              the trend + threshold editor for that metric. */}
          {instance.target_config?.metric_name && instance.target_config?.asset_id && (
            <button
              onClick={e => {
                e.stopPropagation()
                const tc = instance.target_config
                const q = new URLSearchParams({ metric: String(tc.metric_name) })
                if (tc.column) q.set('column', String(tc.column))
                navigate(`/metrics/${tc.asset_id}?${q.toString()}`)
              }}
              className="flex items-center gap-1 text-xs text-primary-600 hover:text-primary-800 font-medium"
              title="View metric trend + tune thresholds"
            >
              <Sparkles className="w-3 h-3" /> View metric
            </button>
          )}
          {isPending && (
...
// ── Anomaly-proposals section ─────────────────────────────────────────────────
// PENDING_PROPOSALS live in a separate table from RULE_INSTANCES — they're
// AI-generated by AnomalyProposalAgent on scheduled scans. Distinct approve/
// reject endpoints (proposalsApi) materialise a RULE_INSTANCE on approval.
// Bell deep-links here via /rule-library?ref=<source_run_id> — matching rows
// get an amber ring so the user's eye lands on them first.
...
function AnomalyProposalRow({
  proposal, highlighted, onReviewed, checked, onCheckedChange, bulkBusy,
}: {
  ...
  const approveMutation = useMutation({
    mutationFn: () => proposalsApi.approve(proposal.id),
    onSuccess: onReviewed,
  })
  const rejectMutation = useMutation({
    mutationFn: (reason: string) => proposalsApi.reject(proposal.id, reason),
    onSuccess: onReviewed,
  })
  const reviewing = approveMutation.isPending || rejectMutation.isPending || bulkBusy
  const handleReject = () => {
    const reason = window.prompt('Reason for rejecting this proposal? (Feeds AI memory to prevent re-proposal.)')
    if (reason && reason.trim()) rejectMutation.mutate(reason.trim())
  }
  const target = [proposal.database_name, proposal.schema_name, proposal.table_name].filter(Boolean).join('.')
  const focusRing = highlighted ? 'ring-2 ring-amber-400 dark:ring-amber-500' : ''

  return (
    <div className={`flex items-center gap-3 px-4 py-3 ${focusRing}`}>
      <input
        type="checkbox"
        checked={checked}
        ...
        aria-label={`Select proposal ${proposal.template_shape}`}
      />
      {proposal.severity && (
        <span className={`text-xs font-semibold px-2 py-0.5 rounded border ${SEVERITY_COLORS[proposal.severity] ?? ''}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          <p className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">
            {shapeLabel(proposal.template_shape)}
            {proposal.metric_name ? ` · ${proposal.metric_name}` : ''}
          </p>
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 flex items-center gap-1">
            <Sparkles className="w-2.5 h-2.5" /> AI-proposed
          </span>
        </div>
        <p className="text-xs text-gray-500 dark:text-gray-400 truncate mt-0.5">
          {target}{proposal.column_name ? ` · ${proposal.column_name}` : ''}
        </p>
        {proposal.rationale && (
...
function AnomalyProposalsSection() {
  const queryClient = useQueryClient()
  const [searchParams] = useSearchParams()
  const refRunId = searchParams.get('ref')

  const { data } = useQuery({
    queryKey: ['anomaly-proposals-pending'],
    queryFn: () => proposalsApi.listPending(200).then(r => r.data),
    staleTime: 15_000,
  })
  ...
  const onReviewed = () => {
    queryClient.invalidateQueries({ queryKey: ['anomaly-proposals-pending'] })
    queryClient.invalidateQueries({ queryKey: ['rule-definitions'] })
    queryClient.invalidateQueries({ queryKey: ['rules-stats'] })
  }

  // Selection drops any id that isn't in the current pending list anymore
  // (e.g. after a bulk approve invalidates the query, some rows disappear).
  useEffect(() => {
    const pendingIds = new Set(proposals.map(p => p.id))
    setSelected(prev => {
      const next = new Set<string>()
      prev.forEach(id => { if (pendingIds.has(id)) next.add(id) })
  ...
  const toggleAll = () => {
    setSelected(allChecked ? new Set() : new Set(proposals.map(p => p.id)))
  }
  ...
  const approveSelected = async () => {
    ...
    try {
      const res = await proposalsApi.approveBatch(ids)
      if (res.data.failed > 0) {
        const firstError = res.data.results.find(r => !r.ok)?.error
        setBulkError(
          `${res.data.failed} of ${ids.length} could not be approved` +
          (firstError ? ` — first error: ${firstError}` : '.')
        )
      }
    } catch (e: any) {
      setBulkError(e?.response?.data?.detail || 'Batch approve failed')
    } finally {
      setBulkBusy(false)
      setSelected(new Set())
      onReviewed()
  ...
  const rejectSelected = async () => {
    ...
    const reason = window.prompt(
      `Reason for rejecting ${selected.size} proposal(s)? (Feeds AI memory to prevent re-proposal.)`
    )
    ...
    const ids = Array.from(selected)
    try {
      const res = await proposalsApi.rejectBatch(ids, reason.trim())
      if (res.data.failed > 0) {
        const firstError = res.data.results.find(r => !r.ok)?.error
        setBulkError(
          `${res.data.failed} of ${ids.length} could not be rejected` +
          (firstError ? ` — first error: ${firstError}` : '.')
        )
      }
    } catch (e: any) {
      setBulkError(e?.response?.data?.detail || 'Batch reject failed')
    } finally {
      setBulkBusy(false)
      setSelected(new Set())
      onReviewed()
  ...
  return (
    <div className="bg-purple-50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-800/50 rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-purple-200 dark:border-purple-800/50 flex items-center gap-3">
        <input
          type="checkbox"
          checked={allChecked}
          ...
          aria-label="Select all anomaly proposals"
        />
        <Sparkles className="w-4 h-4 text-purple-600 dark:text-purple-400" />
        <span className="text-sm font-semibold text-purple-800 dark:text-purple-200">
          {proposals.length} AI-proposed anomaly rule{proposals.length !== 1 ? 's' : ''} awaiting review
        </span>
        {refRunId && (
          <span className="text-xs text-purple-600 dark:text-purple-300">
            (highlighting proposals from the notification you opened)
          </span>
        )}
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-lg bg-green-600 text-white hover:bg-green-700 disabled:opacity-40 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            disabled={bulkBusy || selected.size === 0}
            className="flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-lg border border-red-300 dark:border-red-500/40 text-red-700 dark:text-red-300 hover:bg-red-50 dark:hover:bg-red-950/40 disabled:opacity-40 disabled:cursor-not-allowed"
          >
      ...
      {bulkError && (
        <div className="px-4 py-2 text-xs text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-950/30 border-b border-red-200 dark:border-red-800/50">
          {bulkError}
        </div>
      )}
      <div className="divide-y divide-purple-200/60 dark:divide-purple-800/40">
        {proposals.map(p => (
          <AnomalyProposalRow
            key={p.id}
            proposal={p}
            highlighted={!!refRunId && p.source_run_id === refRunId}
...
function DefinitionsView({ onSelect, highlightId }: { onSelect: (d: RuleDefinition) => void; highlightId?: string | null }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* AI-proposed anomaly rules (from scheduled scans) — separate box so
          they're distinguishable at a glance from manually-added rules. */}
      <AnomalyProposalsSection />
