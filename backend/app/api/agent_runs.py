...
def _workflow_response(w) -> dict:
    return {
        ...
        "created_at": w.created_at.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z' if w.created_at else None,
        "updated_at": w.updated_at.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z' if w.updated_at else None,
